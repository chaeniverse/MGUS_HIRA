...
# median
median1 = round(surv_median(fit_to_MM_6mths)[1,2]+0.5,2)
median2 = round(surv_median(fit_to_MM_6mths)[2,2]+0.5,2)
median3 = round(surv_median(fit_to_MM_6mths)[3,2]+0.5,2)
...
surv1 = paste0(round(surv10[1,3]*100,2),"%")
surv2 = paste0(round(surv10[2,3]*100,2),"%") 
surv3 = paste0(round(surv10[3,3]*100,2),"%")

# ?data.frame

HR1 = paste0(round(sum_cox_fit$conf.int[1,1],2)," (",round(sum_cox_fit$conf.int[1,3],2),",",round(sum_cox_fit$conf.int[1,4],2),") ")
HR2 = paste0(round(sum_cox_fit$conf.int[2,1],2)," (",round(sum_cox_fit$conf.int[2,3],2),",",round(sum_cox_fit$conf.int[2,4],2),") ")
p_val1 = ifelse(sum_cox_fit$coefficients[1,6]<0.001,"p <.001",paste0("p = ", round(sum_cox_fit$coefficients[1,6],2)))
p_val2 = ifelse(sum_cox_fit$coefficients[2,6]<0.001,"p <.001",paste0("p = ", round(sum_cox_fit$coefficients[2,6],2)))

df <- data.frame(`Median survival\n(years)`=c(median1,median2,median3),
                 `HR (95% CI)`=c(HR1, HR2, "Reference"),
                 `P-value`=c(p_val1, p_val2, ""),
                 row.names=c("MGUS to symMM","smolMM to symMM","de novo symMM"), check.names=FALSE)
...
dev.new()
pdf('0823_ggsurvplot_6mths.pdf',height=7,width=10)
...
p
vp <- viewport(x=0.82, y=0.88, width=0.15, height=0.1, just=c("right","top"))
...
scoring_MM <- scoring_MM %>% 
  ...
  mutate(
    ...
    `Low-intensity triplet` = vmp,
    `High-intensity triplet` = vtd
  ) %>% 
  ungroup()

scoring_MM <- rename(scoring_MM, `Cci score`="score")

### using method=cbps ------
set.seed(1)
# warning!!! it takes a long time
w.out <- weightit(group ~ `Cci score` + `Doublet` + `Low-intensity triplet` + `High-intensity triplet`, data = scoring_MM, method="cbps") 
...
library(cobalt)
dev.new()
love.plot(bal.tab(w.out))
ggsave("0823_loveplot.pdf", height=7, width=10,dpi=300)
dev.off()
...
# median
median1 = round(surv_median(fit_scoring_MM_6mths)[1,2]+0.5,2)
median2 = round(surv_median(fit_scoring_MM_6mths)[2,2]+0.5,2)
median3 = round(surv_median(fit_scoring_MM_6mths)[3,2]+0.5,2)

### 6mths landmark version
# use cox to compare
scoring_MM_6mths_filtered$group = relevel(scoring_MM_6mths_filtered$group, ref=3)
cox_fit <- coxph(Surv(time=death_year, event)~group, data=scoring_MM_6mths_filtered, robust=TRUE, weights=w_6mths)
sum_cox_fit <- summary(cox_fit)

# # use log-rank test for p-value
# design_weights <- svydesign(id=~1, strata=NULL, weights=~w_6mths, data=scoring_MM_6mths_filtered)
# 
...
##################
dev.new()
pdf('0823_Adj_ggsurvplot_6mths.pdf',height=7,width=10)
...
# 3mths landmark
mgus_to_mm_3mths <- subset(mgus_to_mm, !(time_yr <=0.25 & status==1))
mgus_to_mm_3mths$time_yr = mgus_to_mm_3mths$time_yr - 0.25
# remove negative values
mgus_to_mm_3mths_filtered <- mgus_to_mm_3mths[mgus_to_mm_3mths$time_yr >=0,]

ci.mgus <- cuminc(ftime=mgus_to_mm_3mths_filtered$time_yr, fstatus=mgus_to_mm_3mths_filtered$status, cencode=0)
...
result_cmprsk <- with(mgus_to_mm_3mths_filtered, cuminc(time_yr, status, cencode=0))
ci.mgus_prod <- prodlim(Hist(time_yr, status, cens.code=0) ~ 1, data=mgus_to_mm_3mths_filtered)

dev.new()
pdf("0823_cuminc_NewlydiagnosedMGUScohort_to_MGUStosymMM_3mths.pdf",height=7,width=10)
plot(ci.mgus_prod,
...
text(10,0.15,pos=2,paste0("10 year\ncumulative incidence\n", ci_10y, " %"),cex=1)
...
# 3mths landmark
smm_3mths <- subset(final_smm_cohort, !(time_yr <= 0.25 & status==1))
smm_3mths$time_yr = smm_3mths$time_yr - 0.25
# remove negative values
smm_3mths_filtered <- smm_3mths[smm_3mths$time_yr >=0.25,]

ci.smm <- cuminc(ftime=smm_3mths_filtered$time_yr, fstatus=smm_3mths_filtered$status, cencode=0)
citable <- ci.smm$`1 1` %>% as.data.table
ci_10y <- sprintf("%.2f", citable[time>9.75][1,2]*100)
ci_half <- as.numeric((citable[time>9.75][1,2])/2)
ci_half_time <- citable[est > ci_half][1,1]

result_cmprsk <- with(smm_3mths_filtered, cuminc(time_yr, status, cencode=0))
ci.smm_prod <- prodlim(Hist(time_yr, status, cens.code = 0)~1, data=smm_3mths_filtered)

pdf("0823_cuminc_NewlydiagnosedsMMcohort_smolMMtosymMM_3mths.pdf",height=7,width=10)
plot(ci.smm_prod,
...
final_smm_cohort <- read_sas("final_smm_cohort.sas7bdat") #4107
...
# 3mths landmark
smm_surv_3mths <- subset(smm_surv, !(time_yr <=0.25 & status==1))
smm_surv_3mths$time_yr = smm_surv_3mths$time_yr - 0.25
# remove negative values
smm_surv_3mths_filtered <- smm_surv_3mths[smm_surv_3mths$time_yr>=0,]

ci.smm_surv <- cuminc(ftime=smm_surv_3mths_filtered$time_yr, fstatus=smm_surv_3mths_filtered$status, cencode=0)
citable <- ci.smm_surv$`1 1` %>% as.data.table
ci_10y <- sprintf("%.2f", citable[time>9.75][1,2]*100)
ci_half <- as.numeric((citable[time>9.75][1,2])/2)
ci_half_time <- citable[est > ci_half][1,1]

result_cmprsk <- with(smm_surv_3mths_filtered, cuminc(time_yr, status, cencode=0))
ci.smm_prod <- prodlim(Hist(time_yr, status, cens.code = 0)~1, data=smm_surv_3mths_filtered)

pdf("0823_cuminc_NewlydiagnosedsmolMMorsymMMcohort_to_smolMMtosymMM_3mths.pdf",height=7,width=10)
plot(ci.smm_prod,
     xlim=c(0.25,10),ylim=c(0,0.2),
     legend.x="topleft",
     legend.cex=1,
